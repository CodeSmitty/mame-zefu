...
RSpec.describe Feature do
  ...
  describe '.recipe_scaling_enabled?' do
    ...
    it 'returns true when main flag is enabled and disabled flag is off' do
      allow(Flipper).to receive(:enabled?).with(:recipe_scaling, user).and_return(true)
      allow(Flipper).to receive(:enabled?).with('recipe_scaling_disabled', user).and_return(false)

      expect(described_class.recipe_scaling_enabled?(user)).to be(true)
    end

    it 'returns false when main flag is disabled' do
      allow(Flipper).to receive(:enabled?).with(:recipe_scaling, user).and_return(false)
      allow(Flipper).to receive(:enabled?).with('recipe_scaling_disabled', user).and_return(false)

      expect(described_class.recipe_scaling_enabled?(user)).to be(false)
    end

    it 'returns false when user is explicitly disabled' do
      allow(Flipper).to receive(:enabled?).with(:recipe_scaling, user).and_return(true)
      allow(Flipper).to receive(:enabled?).with('recipe_scaling_disabled', user).and_return(true)
